...
<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\frac{\mathrm{D} \mathbf{v}}{\mathrm{D} t} = \frac{1}{\rho_0}\nabla_0 \cdot \mathbf{P} + \mathbf{g}" /> (129)
</p>
...
<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\mathbf{F}_a = \sum_b \frac{m_{0b}}{\rho_{0b}} (\mathbf{x}_b - \mathbf{x}_a) \otimes \mathbf{L}_{0a}^{-1} \nabla_{0a} W_{0ab}" /> (132)
</p>

The relevant strain measure in the Total Lagrangian frame is the Green-Lagrange strain, given by:

<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\mathbf{E} = \frac{1}{2}(\mathbf{F}^T \mathbf{F} - \mathbf{I})" /> (133)
</p>
...
<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\mathbf{S} = \lambda \mathrm{tr}(\mathbf{E})\mathbf{I} + 2 \mu \mathbf{E}" /> (134)
</p>

where *λ* and *μ* are the Lamé coefficients, given by:

<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\lambda = \frac{E \nu}{(1+\nu)(1-2\nu)}, \qquad \mu = \frac{E}{2(1+\nu)}" /> (135)
</p>

where *E* is the Young's modulus and *ν* is the Poisson ratio. Finally, the PK1 stress is obtained via:

<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\mathbf{P} = \mathbf{F} \mathbf{S}" /> (136)
</p>
...
<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\begin{align*}
	\frac{\mathrm{D} \mathbf{v}_a}{\mathrm{D} t} = 
	&- \sum_{b \in b_f} m_b \left(\frac{p_a + p_b}{\rho_a \rho_b}\right) \nabla_a W_{ab} + \sum_{b \in b_f} m_b \left(\frac{4 \nu \mathbf{x}_{ab} \cdot \nabla_a W_{ab}}{(\rho_a + \rho_b)(x_{ab}^2 + \eta^2)}\right) \mathbf{v}_{ab} \\
	&- \sum_{b \in b_s} m_b \left(\frac{p_a + p_b}{\rho_a \rho_b}\right) \nabla_a W_{ab} + \sum_{b \in b_s} m_b \left(\frac{4 \nu \mathbf{x}_{ab} \cdot \nabla_a W_{ab}}{(\rho_a + \rho_b)(x_{ab}^2 + \eta^2)}\right) \mathbf{v}_{ab} \\
	&- \sum_{b \in b_b} m_b \left(\frac{p_a + p_b}{\rho_a \rho_b}\right) \nabla_a W_{ab} + \sum_{b \in b_b} m_b \left(\frac{4 \nu \mathbf{x}_{ab} \cdot \nabla_a W_{ab}}{(\rho_a + \rho_b)(x_{ab}^2 + \eta^2)}\right) \mathbf{v}_{ab} + \boldsymbol{\tau}^{SPS}_a + \mathbf{g}_a
\end{align}" /> (137)
</p>

where *b*<sub>f</sub>, *b*<sub>s</sub> and *b*<sub>b</sub> are sets containing the neighbouring fluid, structure and boundary particles for particle *a*. Similarly, the acceleration of structure particle *a* is:

<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\begin{align*}
	\frac{\mathrm{D} \mathbf{v}_a}{\mathrm{D} t} = 
	&- \frac{m_a}{m_{0a}} \sum_{b \in b_f} m_b \left(\frac{p_a + p_b}{\rho_a \rho_b}\right) \nabla_a W_{ab} + \frac{m_a}{m_{0a}} \sum_{b \in b_f} m_b \left(\frac{4 \nu \mathbf{x}_{ab} \cdot \nabla_a W_{ab}}{(\rho_a + \rho_b)(x_{ab}^2 + \eta^2)}\right) \mathbf{v}_{ab} \\
	&+ \sum_{b \in b_s} m_{0b} \left(\frac{\mathbf{P}_a \mathbf{L}_{0a}^{-1}}{\rho_{0a}^2} + \frac{\mathbf{P}_b \mathbf{L}_{0b}^{-1}}{\rho_{0b}^2}\right) \cdot \nabla_{0a} W_{0ab} + \frac{\mathbf{f}_a^{HG}}{m_{0a}} + \mathbf{g}_a
\end{align}" /> (138)
</p>
...
<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\mathbf{v}_a^{n+1} = \mathbf{v}_a^n + \Delta t \frac{\mathrm{D} \mathbf{v}_a}{\mathrm{D} t}^n, \qquad \mathbf{x}_a^{n+1} = \mathbf{x}_a^n + \Delta t \mathbf{v}_a^{n+1}" /> (139)
</p>

Finally, the structure phase also introduces an additional restriction on the timestep. The new timestep criterion becomes:

<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\Delta t = \alpha_c \min(\Delta t_a, \Delta t_{cv}, \Delta t_s)" /> (140)
</p>

where:

<p align="center">
<img src="http://latex.codecogs.com/png.image?\dpi{150}\bg{white}\Delta t_s = \min_a\left(\frac{h}{\sqrt{\frac{E_a (1 - \nu_a)}{\rho_{0a}(1 + \nu_a)(1 - 2\nu_a)}}}\right)" /> (141)
</p>
